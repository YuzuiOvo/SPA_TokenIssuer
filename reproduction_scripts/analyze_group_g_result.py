...
FIG_PATH = os.path.join(OUTPUT_DIR, "group_g_verification_chart.png")
...
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width() / 2.0, yval + 0.5, f"{int(yval)}", ha='center', va='bottom', fontsize=10)

plt.title("Group G Verification Result (Missing Token Test)", fontsize=14)
